...
from problem import check_outer_bag_can_contain_inner, count_num_inner_bags,\
    count_num_possible_containing_bags, parse_rules
...
def test_day7_num_inner_bags(rules):
    assert count_num_inner_bags(rules, 'shiny gold') == 32
    assert count_num_inner_bags(parse_rules(ALTERNATIVE_RULES), 'shiny gold') == 126
